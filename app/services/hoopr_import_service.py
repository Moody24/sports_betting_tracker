...
def _parse_plus_minus(value) -> float:
    ...
    if isinstance(value, str):
        return safe_float(value.replace('+', '').strip() or None)
...
def _rows_from_player_box(df, season: str, season_type_code: int,
    ...
    for rec in played.to_dict('records'):
        stats = {
            key: safe_float(rec.get(col))
            for col, key in _HOOPR_STAT_COLUMNS.items()
        }
        stats['plus_minus'] = _parse_plus_minus(rec.get('plus_minus'))
        stats['team_score'] = safe_float(rec.get('team_score'))
        stats['opp_score'] = safe_float(rec.get('opponent_team_score'))
        ...
        rows.append(dict(
            sport='nba',
            player_id=normalize_player_id(rec.get('athlete_id')),
            player_name=safe_str(rec.get('athlete_display_name')),
            team_abbr=safe_str(rec.get('team_abbreviation')) or None,
            opp_abbr=safe_str(rec.get('opponent_team_abbreviation')) or None,
            game_id=safe_str(rec.get('game_id')),
            game_date=_as_date(rec.get('game_date')),
            season=season,
            home_away=safe_str(rec.get('home_away')).upper() or None,
...
def import_hoopr_seasons(sport='nba', seasons=3,
    ...
    season_type_code = SEASON_TYPE_CODES[season_type]
