...
- `FEATURE_KEYS` in `ml_feature_builder.py` is the canonical shared feature list — must stay in sync between training and inference (currently 30 features)
...
- Own 21-feature set (not Model 1's), independently defined in `pick_quality_model.py`: 17 `PICK_FEATURES` + 4 encoded categoricals (`player_trend`, `minutes_trend`, `confidence_tier_num`, `injury_returning`), including 2 volatility features: `minutes_volatility`, `stat_attempts_volatility`
